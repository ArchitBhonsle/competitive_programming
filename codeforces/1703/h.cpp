...
void solve(int testcase) {
  ...
  // Using all the good keys first followed by all the bad keys is always more
  // profitable.
  //
  // This means there will be a prefix of chest opened by good keys, followed by
  // all the chests opened by bad chests.
  //
  // Thus, we check all the possible prefixes of good chests and select the best
  // one.

  vll good_prefix_gain = {0};
  fo(i, 0, n) {
    ll gain = good_prefix_gain[i] + a[i] - k;
    good_prefix_gain.push_back(gain);
  }

  ll res = 0;
  fo(i, 0, n + 1) { // here i is the end of the good prefix
    ll bad_suffix_gain = 0;
    fo(j, i, min(i + 31, n)) {
      ll curr_bad_gain = a[j] >> (j - i + 1);
      bad_suffix_gain += curr_bad_gain;
    }

    deb(i, good_prefix_gain[i + 1], bad_suffix_gain);
    res = max(res, good_prefix_gain[i] + bad_suffix_gain);
  }
